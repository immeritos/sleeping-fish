...
export default function CursorOverlay() {
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (el) {
        const clickableSelector = "a, button, [role='button'], [onclick], .cursor-pointer, input, select, textarea";
  ...
  const style: React.CSSProperties = {
    ...
    boxShadow: shouldEmphasize
      ? `0 0 ${baseSize * 2}px ${isDark ? "rgba(51, 88, 212, 0.15)" : "rgba(255, 243, 148, 0.25)"}`
      : `0 0 ${baseSize}px ${isDark ? "rgba(51, 88, 212, 0.15)" : "rgba(255, 243, 148, 0.25)"}`,
    opacity,
    pointerEvents: "none",
    zIndex: 9999,
